...
export function canColorHighlight(
  ...
  if (mode === "mark") {
    ...
    try {
      // Some tiptap setups don't include `toggleNodeBackgroundColor` on the typed `CanCommands`.
      // Cast to `any` to avoid the TypeScript error while still performing the runtime check.
      return (editor as any).can().toggleNodeBackgroundColor("test");
    } catch {
      return false;
    }
  }
